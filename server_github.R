...
get.dataSet <- function(data.name="Hess-133", compound.name="paclitaxel"){
  ...
  if( data.name=="OV:TCGA-448"){
    evaluationset.clinic <- subset(TCGA448.clinic, adjuvant.paclitaxel==1)
    if( tolower(compound.name)=="cyclophosphamide" ) evaluationset.clinic <- subset(TCGA448.clinic, adjuvant.cytoxan==1)
    evaluationset.clinic$is.valid=1
    rname <- rownames(evaluationset.clinic)
    evaluationset <- TCGA448[,rname] 
    evaluationset.response <- setNames(evaluationset.clinic[rname, "response.binary"], rname)
  }
  
  
  if( data.name=="OV:Bonome185"){
    
    load("./Combo Chemo Data/ovarian/ovarian185.total.RData")
    rownames(ovarian185.clinic) <- paste( ovarian185.clinic$SO, ".CEL", sep="")
    colnames(ovarian185) <- rownames(ovarian185.clinic)
    
    ovarian185.clinic$is.paclitaxel <- as.numeric( ovarian185.clinic$chemotype 
                                                   %in% c("Cisplatin/Taxol", "Taxol/Carbo", "Taxol/Carbo/Gem", "Taxol/Cytoxan/Carbo"))
    ovarian185.clinic$is.cisplatin <- as.numeric( ovarian185.clinic$chemotype 
                                                  %in% c("Cisplatin/Taxol"))
    ovarian185.clinic$is.cytoxan <- as.numeric( ovarian185.clinic$chemotype 
                                                %in% c("Carbo/Cytoxan", "Taxol/Cytoxan/Carbo") )
    ovarian185.clinic$response.ordinal = as.numeric(recode( ovarian185.clinic$PrimaryChemoOutcome, 
                                                            recode="c('Complete Response')=3;c('Partial Response')=2;c('Progression of Disease')=1;else=NA")) #, 
    #as.numeric.result = TRUE)) ## to make numbers
    ovarian185.clinic$response.binary <- ovarian185.clinic$response

    evaluationset.clinic <- subset(ovarian185.clinic, is.paclitaxel==1)
    
    if( tolower(compound.name)=="cyclophosphamide" ) evaluationset.clinic <- subset(ovarian185.clinic, is.cytoxan==1)
    if( tolower(compound.name)=="cisplatin" ) evaluationset.clinic <- subset(ovarian185.clinic, is.cisplatin==1)
    
    evaluationset.clinic$is.valid=1
    rname <- rownames(evaluationset.clinic)
    evaluationset <- ovarian185[,rname] 
    evaluationset.response <- setNames(evaluationset.clinic[rname, "response.binary"], rname)
  }
  ...
  if( data.name=="CRC:Khambata(Cetuximab)"){
    ...
    ## align PFS
    #GSE5851.clinic$pfs <- GSE5851.clinic$Progression.free.survival...of.days
    #GSE5851.clinic$progression <- 1
  ...
  if(data.name=="HNSCC_GSE65021(Illumina)_Cetuximab"){
    x <- readRDS(file="../Combo Chemo Data/hnscc/GSE65021/GSE65021.u133p2.mean.by.genesymbol.rds")
    x.clinic <- readRDS(file="../Combo Chemo Data/hnscc/GSE65021/GSE65021.u133p2.clinic.mean.by.genesymbol.rds")
    
    id <- 1:ncol(x)      
    evaluationset <- x[,id]
    evaluationset.clinic <- x.clinic[id, ]
    evaluationset.response <- evaluationset.clinic$response
    evaluationset.name=paste("HNSCC:GSE60521(Illumina):Cetuximab-", ncol(evaluationset))
  }    
  ...
  ## pancreatic/GSE9599
  ## "./Combo Chemo Data/pancreatic/GSE9599"
  if( data.name=="Pancreatic:GSE9599(Erlotinib)"){
    cat(compound.name)
    
    total <- readRDS(file="../Combo Chemo Data/pancreatic/GSE9599/EMTAB9599.total.rds")
    id <- which(total$clinic$Role=="training")
    
    evaluationset <- total$expr[,id]
    evaluationset.clinic <- total$clinic[id,]
    evaluationset.response <- evaluationset.clinic$response[id]
    cat("Finish initializing test Data set\n")
    #if( tolower(compound.name)=="erlotinib"  ){
    evaluationset.clinic$response.ordinal <- evaluationset.clinic$response.Erlotinib
    
    ### if an ordinal response is valid, use it for validating a GEM of interest. 
    if( !is.null(evaluationset.clinic$response.ordinal)){
      evaluationset.response <- evaluationset.clinic$response.ordinal
    }
  }
  
  if( data.name=="Pancreatic:GSE9599(Cetuximab)"){
    cat(compound.name)
    ...
    evaluationset.response <- evaluationset.clinic$response[id]
    cat("Finish initializing test Data set\n")
    evaluationset.clinic$response.ordinal <- evaluationset.clinic$response.Cetuximab
  ...
  ## DLBCL: GSE17920: Relapse outcome
  if( data.name=="DLBCL_GSE17920"){
    total <- readRDS(file="~/Documents/COXEN/Combo Chemo Data/lymphoma/GSE17920/GSE17920.total.rds")
    evaluationset <- total$expr
    evaluationset.clinic <- total$clinic
    evaluationset.response <- total$clinic$response
  }
  ...
  if( data.name=="NSCLC:BATTLE(Sorafenib|Erlotinib)"){
    ...
    #id.outlier <- c(52,55,57,92,114,127)
    #x <- x[,-id.outlier]
    #x.clinic <- x.clinic[-id.outlier, ]    
    
    #cbind(colnames(x), rownames(x.clinic))
    #table(x.clinic$treatment) ## 25 erlotinib; 1 erlotinib+bexaroten;39 sorafenib; 62 NAs.
    #x.clinic$PFSM.MONTH
    #x.clinic$PROGRESSION
    #x.clinic$HISTOLOGY.IW.REVISED
    ## select cases with data on drug response
    id <- 1:nrow(x.clinic)
    if( tolower(compound.name)=="erlotinib"){
  ...
  #### Adjuvant Cisplatin+Vinorelbine (ACT) or Observed cohorts (OBS)
  if( data.name=="NSCLC:GSE14814:Cisplatin+Vinorelbine"){
    x <- readRDS(file="./Combo Chemo Data/lung/GSE14814-cisplatin+vinorelbine/GSE14814.RDS")
    x.clinic <- readRDS( file="./Combo Chemo Data/lung/GSE14814-cisplatin+vinorelbine/GSE14814.clinic.RDS")
    
    id <- 1:nrow(x.clinic)
    id <- which( x.clinic$POST.SURGICAL.TREATMENT=="ACT")
    evaluationset <- x[, id]
    evaluationset.clinic <- x.clinic[id, ]
    evaluationset.response <- NULL
  ...
  if( data.name=="NSCLC:GSE14814:OBS"){
    x <- readRDS(file="./Combo Chemo Data/lung/GSE14814-cisplatin+vinorelbine/GSE14814.RDS")
    x.clinic <- readRDS( file="./Combo Chemo Data/lung/GSE14814-cisplatin+vinorelbine/GSE14814.clinic.RDS")
    
    id <- 1:nrow(x.clinic)
    id <- which( x.clinic$POST.SURGICAL.TREATMENT=="OBS")
    evaluationset <- x[, id]
    evaluationset.clinic <- x.clinic[id, ]
    evaluationset.response <- NULL
  ...
  ### Melanoma 
  if( data.name=="Melanoma:ETABM760:dacarbazine+sorafenib"){
    ##total <- list(expr=expr, expr.gene=expr.gene, clinic=clinic, anno=anno)
    #total <- readRDS(file="~/Documents/COXEN/Combo Chemo Data/PanCancer/E-TABM-760/ETABM760.total.rds")
    total <- readRDS(file="~/Documents/COXEN/Combo Chemo Data/PanCancer/E-TABM-760/ETABM760.total.u133p2.rds")
    total$clinic$time <- total$clinic$Sample.Name
    x.clinic <- total$clinic
    x <- total$expr.u133p2
    
    ### All patients received the combiantion of dacarbazine and sorafenib, and also replication.
    if( tolower(compound.name) %in% c("dacarbazine", "sorafenib") ){
      #id <- which(x.clinic$treatment=="none,none" & x.clinic$Label=="Cy5" & x.clinic$OrganismPart=="liver") ## to get pretreatment expression
      id <- which(x.clinic$treatment=="none,none" & x.clinic$Label=="Cy3" & x.clinic$Histology=="Melanoma") ## to get pretreatment expression
    }else{
      id <- NA
    }
    #x.clinic[id, c("Individual", "treatment", "time", "Label", "tissue")]
    evaluationset <- x[, id]
    evaluationset.clinic <- x.clinic[id, ]
    evaluationset.response <- as.numeric(evaluationset.clinic$response)
  }
  ...
  ### PanCancer    
  if( data.name=="PanCancer:ETABM760:dacarbazine+sorafenib"){
    ##total <- list(expr=expr, expr.gene=expr.gene, clinic=clinic, anno=anno)
    #total <- readRDS(file="~/Documents/COXEN/Combo Chemo Data/PanCancer/E-TABM-760/ETABM760.total.rds")
    total <- readRDS(file="~/Documents/COXEN/Combo Chemo Data/PanCancer/E-TABM-760/ETABM760.total.u133p2.rds")
    total$clinic$time <- total$clinic$Sample.Name
    ...
    if( tolower(compound.name) %in% c("dacarbazine", "sorafenib") ){
      #id <- which(x.clinic$treatment=="none,none" & x.clinic$Label=="Cy5" & x.clinic$OrganismPart=="liver") ## to get pretreatment expression
      id <- which(x.clinic$treatment=="none,none" & x.clinic$Label=="Cy3") ## to get pretreatment expression
    }else{
      id <- NA
    }
    #x.clinic[id, c("Individual", "treatment", "time", "Label", "tissue")]
